...
for pid,p in enumerate(paperdict):
    tw = topdict.get( str(p), [] ) # top 100 words
    ts = [x[0] for x in tw]
    for t in ts:
...
for pid, p in enumerate(paperdict):
    ...
    filename = "%s-%s" % ( p, slug(title) )
    thumbpath = "thumbs/%s.pdf.jpg" % filename

    # get links to PDF, supplementary and bibtex on NIPS servers
    pdflink = "http://papers.nips.cc/paper/%s.pdf" % filename
    bibtexlink = "http://papers.nips.cc/paper/%s/bibtex" % filename
    supplink = "http://papers.nips.cc/paper/%s-supplemental.zip" % filename
